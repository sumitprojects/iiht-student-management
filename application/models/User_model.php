...
class User_model extends CI_Model {
    ...
    public function get_user_by_email_sms($user_id = 0) {
        if ($user_id > 0) {
            $this->db->where('id', $user_id);
        }
        $this->db->select('concat(first_name," ", last_name) as full_name, email');
        $this->db->where('role_id', 2);
        $this->db->where('is_instructor', 0);
        $this->db->where('is_delete',0);
        $this->db->where('email is not null',false,false);
        $users = $this->db->get('users')->result_array();
        $enquiry = $this->db->select('en_name as full_name, en_email as email')->where('en_email is not null',false,false)->get('enquiry')->result_array();
        return array_merge($users,$enquiry); 
    }
        ...
        if ($validity == false) {
            ...
            */
            ...
            if (isset($_FILES['user_image']) && $_FILES['user_image']['name'] != "") {
                $image = $this->db->get_where('users', array('id' => $user_id))->row('image');
                if(!empty($image) && file_exists('uploads/user_image/' . $this->db->get_where('users', array('id' => $user_id))->row('image').'.jpg')){
    ...
    public function add_user($is_instructor = false) {
        ...
        // }else {
            ...
            */
            ...
            $data['source_other'] = html_escape($this->input->post('source_other'));
            ...
            if(!$is_instructor) $this->crud_model->enrol_a_student_offline($enrol['user_id'],$enrol['course_id'],$enrol['price'],$enrol['is_training'],$enrol['enrol_status']);
            $this->upload_user_image($data['image']);
            $this->session->set_flashdata('flash_message', get_phrase('user_added_successfully'));
        // }
    }
    ...
    public  function add_permission($user_id){
        ...
        if(is_array($permission)){
            ...
            $this->db->where('user_id', $user_id);
            $p = $this->db->get('permissions');
            if($p->num_rows() > 0){
                $perm['user_id'] = $user_id;
                $perm['permissions'] = json_encode($permission);
                $this->db->update('permissions',$perm);
            }else{
                $perm['user_id'] = $user_id;
                $perm['permissions'] = json_encode($permission);
