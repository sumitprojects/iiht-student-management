...
class Home extends CI_Controller {
    ...
    //adil:exam_quiz
    public function exams_quiz($slug = "", $course_id = "", $lesson_id = "") {
        if ($this->session->userdata('user_login') != 1){
            if ($this->session->userdata('admin_login') != 1){
                redirect('home', 'refresh');
            }
        ...
        $page_data['course_details']  = $course_details;
        $page_data['course_id']  = $course_id;
        $page_data['page_name']  = 'exam_quiz';
        $page_data['page_title'] = $course_details['title'];
        $this->load->view('lessons/index2', $page_data);
    }
    ...
    public function principal() {
        ...
        $page_data['page_title'] = site_phrase('principal');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    public function academic_staff() {
        $page_data['page_name'] = 'academic_staff';
        $page_data['page_title'] = site_phrase('academic_staff');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    public function non_academic_staff() {
        $page_data['page_name'] = 'non_academic_staff';
        $page_data['page_title'] = site_phrase('non_academic_staff');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function submit_quiz($from = "", $course_id="") {
        $counter = 0;
        if($this->session->userdata('counter')) $counter += $this->session->userdata('counter');
        $totalPhase = $this->db->get_where('lesson',['course_id' => $course_id, 'lesson_type'=>'quiz'])->get()->num_rows();
        ...
        $data['user_id']=$this->session->userdata('user_id');
        $submitteddata = [];
        if($this->session->userdata('submit_que')){
            array_push($submitteddata,$this->session->userdata('submit_que'));
        }
        // array_push($submitteddata,$data);
        
        $this->session->set_userdata('submit_que' , $submitteddata);
        $this->session->set_userdata('final_submit' , $submitteddata);
        var_dump($submitteddata);
            die;
        if($this->session->userdata('submit_que') && $counter == $totalPhase){
    ...
    public function payment_gateway_mobile($course_id = "", $user_id = ""){
     ...
     public function event(){
        ...
        $page_data['page_title'] = 'event list';
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    
    public function directors(){
        $page_data['page_name'] = 'director_list';
        $page_data['page_title'] = 'Directors';
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function student_view($param1 = '' , $param2 = '',$param3 = ""){
            ...
            else if(($param1== 'present' || $param1== 'pending') && $param2 > 0 && $param3 > 0){
               $this->db->update('attendance',['att_status'=>$param1],['id' => $param2]);
               redirect(site_url('home/student_view/request/'.$param3), 'refresh');
            }
            else{
        ...
        if($param1 == 'register'){
            ...
            $data['message']   = html_escape($this->input->post('message'));
             $result=$this->db->get_where('contact_us',array('email'=>$data['email']))->num_rows();
            if ($result==0) {
        ...
        $page_data['page_name'] = 'contact_page';
        $page_data['page_title'] = 'contact form';
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    public function manage_attendance($param1 = ""){
        if($param1 == 'add'){
            $enrol_id=$_POST['en_id'];
                foreach($enrol_id as $en_id){
                    ...
                    redirect(site_url('home/student_view/'), 'refresh');
        }
        $page_data['enrol_list'] =$this->crud_model->get_enrol_data("",$this->session->userdata('user_id'))->result_array();
        $page_data['page_name'] = 'add_attendance';
        $page_data['page_title'] = 'add attendance';
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    
    public function career($param1 = ""){
        if($param1 == 'apply'){
            $this->crud_model->post_application();
            //redirect(site_url('home/career/'), 'refresh');
        }
        $page_data['page_name'] = 'career';
        $page_data['page_title'] = 'Career With US';
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
}